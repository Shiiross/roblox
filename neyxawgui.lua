if game.PlaceId == 139609232261520 then
local ui = loadstring(game:HttpGet("https://raw.githubusercontent.com/Shiiross/roblox/refs/heads/main/important.lua"))()

local win = ui:Create({
    Name = "Neyxaw's GUI",
    ThemeColor = Color3.fromRGB(255, 140, 0),
	ThemeGradient = Color3.fromRGB(80, 30, 0),
...
local visualFolder = esptab:Folder("ESP & Göstergeler.", Color3.fromRGB(255, 0, 255), true)
visualFolder:Label("Neyxaw'ın gözü açıldı! (Drawing ESP)")
...
local ESPObjects = {}
local Settings = { ESP = ESP }
local LocalPlayer = game.Players.LocalPlayer
local CurrentCamera = workspace.CurrentCamera

-- Rol ve renk fonksiyonları
local function GetPlayerRole(player)
    local char = player.Character
    if char then
        local state = char:FindFirstChild("state")
        if state then
            local roleVal = state:FindFirstChild("role")
            if roleVal then
...
local function GetRoleColor(role)
    local colors = {
        ["Forward"] = Color3.fromRGB(255, 100, 100),
        ["Midfielder"] = Color3.fromRGB(100, 255, 100),
        ["Defender"] = Color3.fromRGB(100, 100, 255),
        ["Goalkeeper"] = Color3.fromRGB(255, 255, 100),
...
local function UpdateESP()
    -- Bağlantısı kesilmiş oyuncuları temizle
    ...
    for _, player in pairs(game.Players:GetPlayers()) do
        ...
        if player.Team == LocalPlayer.Team then
            color = Color3.fromRGB(0, 255, 100) -- Takım arkadaşı yeşil
        else
            color = Color3.fromRGB(255, 50, 50) -- Rakip kırmızı
        end
...
local function StartESP()
    ...
    renderConn = game:GetService("RunService").RenderStepped:Connect(UpdateESP)
end
...
-- UI Elementleri
visualFolder:Toggle("ESP Aktif", "Oyuncuları ekranda gösterir (Neyxaw artık herkesi görüyor).", function(v)
    ESP.Enabled = v
    if v then
        StartESP()
    else
...
visualFolder:Toggle("Kutu", "Oyuncunun etrafında kutu gösterir.", function(v)
    ESP.Box = v
end)

visualFolder:Toggle("İsim", "Oyuncunun ismini gösterir.", function(v)
    ESP.Name = v
end)

visualFolder:Toggle("Rol", "Oyuncunun rolünü gösterir (Forvet, Orta saha vb.).", function(v)
    ESP.Role = v
end)

visualFolder:Toggle("Mesafe", "Oyuncuya olan mesafeyi gösterir.", function(v)
    ESP.Distance = v
end)

visualFolder:Toggle("Tracer", "Oyuncuya ekranın altından çizgi çeker.", function(v)
    ESP.Tracer = v
end)
...
visualFolder:Label("Not: Takım arkadaşı yeşil, rakip kırmızı görünür.")   
 
hometab:Label("Script Made By Neyxaw!")
...
local role = game:GetService("ReplicatedStorage")
    :WaitForChild("BytenetStorage")
    :WaitForChild("Networking").Value
...
helptab:Label("Main Scripter: Neyxaw.")
...
flowPartData.flowPart.Size = Vector3.new(30, 1, 30)
flowPartData.flowPart.Anchored = true
flowPartData.flowPart.CanCollide = true
flowPartData.flowPart.Position = Vector3.new(100, 1000, 100)
flowPartData.flowPart.Parent = workspace

ballfolder:Button("Get Flow (Req. Ball)", "Creates A Safe Area And Spams Kick To Get Flow. Click Again To Cancel.", function()
    ...
    flowPartData.flowConnection = task.spawn(function()
        local flow = workspace:WaitForChild("characters"):WaitForChild(flowPartData.LocalPlayer.Name):WaitForChild("state"):WaitForChild("flow")
...
nocd:Toggle("No Rush Cooldown", "Emulates Having No Rush Cooldown.", function()
    ...
    notify("No Rush CD", dashState.ForwardRushEnabled and "Enabled." or "Disabled.")
end)
...
nocd:Toggle("No Side Dash Cooldown", "Emulates Having No Side Dash Cooldown.", function()
    ...
    notify("No Side Dash CD", dashState.SideDashEnabled and "Enabled." or "Disabled.")
end)
...
ballmag:Toggle("Ball Magnet", "The Original Ball Magnet, Always On Max Range.", function()
    ...
    if ballMagnetState.active then
        ...
        ballMagnetState.conn = task.spawn(function()
            while ballMagnetState.active do
                pcall(function()
...
local remote2 = game:GetService("ReplicatedStorage"):WaitForChild("ByteNetReliable")
...
ballmag:Toggle("Ball Magnet (Bind: M2, Click / Hold.)", "Similar To The Original, But Only Activate While Right Click Is Held Down.", function()
    if not magnetHoldState.enabled then
        magnetHoldState.enabled = true

        magnetHoldState.beganConnection = userInputService2.InputBegan:Connect(function(input, gameProcessed)
            if gameProcessed then return end
            if input.UserInputType == Enum.UserInputType.MouseButton2 then
                magnetHoldState.holding = true
                task.spawn(function()
                    while magnetHoldState.holding do
                        remote2:FireServer(buffer.fromstring(buffers["grabball"]))
                        task.wait(0.05)
                    end
                end)
            end
        ...
        magnetHoldState.endedConnection = userInputService2.InputEnded:Connect(function(input, gameProcessed)
            if gameProcessed then return end
            if input.UserInputType == Enum.UserInputType.MouseButton2 then
                magnetHoldState.holding = false
            end
        end)
        ...
        magnetHoldState.enabled = false
        magnetHoldState.holding = false

        if magnetHoldState.beganConnection then magnetHoldState.beganConnection:Disconnect() magnetHoldState.beganConnection = nil end
        if magnetHoldState.endedConnection then magnetHoldState.endedConnection:Disconnect() magnetHoldState.endedConnection = nil end
...
ballmag:Toggle("Ball Magnet Hitbox", "Shows The Magnet's Hitbox On The Ball.", function()
    ballHitboxState.active = not ballHitboxState.active

    game.StarterGui:SetCore("SendNotification", {
        Title = "Ball Magnet Hitbox",
        Text = ballHitboxState.active and "Enabled." or "Disabled.",
        Duration = 1
    })

    local function removeHitbox()
        if ballHitboxState.hitbox then
            ballHitboxState.hitbox:Destroy()
            ballHitboxState.hitbox = nil
        end
    end

    local function stopLoop()
        if ballHitboxState.loop then
            ballHitboxState.loop:Disconnect()
            ballHitboxState.loop = nil
        end
    end

    if not ballHitboxState.active then
        stopLoop()
        removeHitbox()
        return
    end
    ...
    ballHitboxState.loop = RunService.Heartbeat:Connect(function()
        if not ballHitboxState.active then
            stopLoop()
            removeHitbox()
            return
        ...
        if not ballHitboxState.hitbox then
...
local remote3 = ReplicatedStorage:WaitForChild("ByteNetReliable")
local localPlayer = Players.LocalPlayer

local ballRadiusState = {
    radius = 1,
    conn = nil,
    lastFire = 0,
    FIRE_INTERVAL = 0.1,
...
ballmag:Slider("Ball Magnet Radius", 1, 1, 25, 0.5, function(v)
    ballRadiusState.radius = v
end)

if ballRadiusState.conn then
    ballRadiusState.conn:Disconnect()
end

local function setupCharacter(char)
    ballRadiusState.hrp = char:WaitForChild("HumanoidRootPart", 10)
end
...
ballRadiusState.conn = RunService.Heartbeat:Connect(function()
    local radius = ballRadiusState.radius or 1
    if radius < 2 then return end

    local ball = Workspace.Terrain:FindFirstChild("Ball")
    if not ball then return end

    if ballRadiusState.hrp then
        local dist = (ballRadiusState.hrp.Position - ball.Position).Magnitude
        if dist <= radius and (time() - ballRadiusState.lastFire) >= ballRadiusState.FIRE_INTERVAL then
            ballRadiusState.lastFire = time()
            pcall(function()
                remote3:FireServer(buffer.fromstring(buffers["grabball"]))
            end)
        end
    end
end)

local ballRangeVizState = {
    enabled = false,
    hitbox = nil,
    loop = nil
}
...
ballmag:Toggle("Ball Magnet Radius Hitbox", "Shows The Magnet's Hitbox On The Ball.", function()
    ballRangeVizState.enabled = not ballRangeVizState.enabled

    game.StarterGui:SetCore("SendNotification", {
        Title = "Ball Magnet Radius Hitbox",
        Text = ballRangeVizState.enabled and "Enabled." or "Disabled.",
        Duration = 1
    })

    local function removeHitbox()
        if ballRangeVizState.hitbox then
            ballRangeVizState.hitbox:Destroy()
            ballRangeVizState.hitbox = nil
        end
    end

    local function stopLoop()
        if ballRangeVizState.loop then
            ballRangeVizState.loop:Disconnect()
            ballRangeVizState.loop = nil
        end
    end

    if not ballRangeVizState.enabled then
        stopLoop()
        removeHitbox()
        return
    end
    ...
    ballRangeVizState.loop = RunService.Heartbeat:Connect(function()
        if not ballRangeVizState.enabled then
            stopLoop()
            removeHitbox()
            return
        ...
        if not ballRangeVizState.hitbox then
            local hitbox = Instance.new("Part")
            ...
            ballRangeVizState.hitbox = hitbox
        end

        local d = radius * 2
        local hitbox = ballRangeVizState.hitbox
    ...
    for i = 1, #playersList do 
        ...
        if plr ~= player and plr.Team == pTeam and plr.Character then 
            ...
            if hrp then 
                ...
                local distX = pPos.X - tPos.X
                local distY = pPos.Y - tPos.Y
                local distZ = pPos.Z - tPos.Z
...
local function onCharacterAdded(char)
    ...
    if not target or target == player or target.Team == player.Team then return end
    if target.Team == player.Team then return end

    if connections[target] then connections[target]:Disconnect() end
    
    connections[target] = animator.AnimationPlayed:Connect(function(animTrack)
        local isTackleAnim = false
        for i = 1, #tackle_anim do
            if animTrack.Animation.AnimationId == tackle_anim[i] then
                isTackleAnim = true
                break
            end
        end
        
        if isTackleAnim then
            if target.Team == player.Team then return end
            ...
            if root and targetHRP then
                ...
                local distZ = pPos.Z - tPos.Z
                local distSq = (distX * distX) + (distY * distY) + (distZ * distZ)
                if distSq > (autoSkillState.mCD * autoSkillState.mCD) then return end
            else
                return
            end
            local now = tick()
            if now - (autoSkillState.cooldowns[target] or 0) >= 0.5 then
                autoSkillState.cooldowns[target] = now
                local closestTeammate = autoSkillState.useClosestTeammate and getClosestTeammate() or nil
                for i = 1, 5 do
                    local toggleKey = toggleKeys[i]
                    if autoSkillState[toggleKey] then
                        local skillName = autoSkillState.autoSkills[toggleKey]
                        if skillName then fireSkill(skillName, closestTeammate) end
                    end
                end
                if autoSkillState.toggleTSpecial then
                    task.delay(0.05, function()
                        local tsName = autoSkillState.autoSkills["toggleTSpecial"]
                        if tsName then fireSkill(tsName, closestTeammate) end
                    end)
                end
                if autoSkillState.toggleDribble then
                    local dribbleName = autoSkillState.autoSkills["toggleDribble"]
                    if dribbleName then fireSkill(dribbleName, closestTeammate) end
                end
            end
        end
    end)
...
autodrib:Slider("Counter Radius", 65, 45, 75, 1, function(v)
    autoSkillState.mCD = v
    notify2("Counter Radius", "Counter Radius: " .. v .. "", 2)
end)
...
autoline:Toggle("Sae Line Ups", "Sae's Drive Shot Line Ups.", function(s)
	if s then
		...
		for i = 1, 8 do
			...
			local basePos = (i <= 4) and agoalPos or bgoalPos
			local col = colors[(i - 1) % 4 + 1]
...
autoline:Toggle("Kaiser Line Ups", "Kaiser's Impact Magnus Line Ups.", function(k)
	if k then
		...
		for i = 1, 12 do
			...
			local basePos = (i <= 6) and bgoalPos or agoalPos
			local col = colors[i]
...
autoline:Toggle("Karasu Line Ups", "Karasu's Dive Bomb Line Ups.", function(kar)
	if kar then
	    ...
	    local goalRelative = {
			{pad = Vector3.new(-47, -9, 164), target = Vector3.new(-25, 30, 14)},
			{pad = Vector3.new(-24, -9, 193), target = Vector3.new(-30, 78, 14)},
			{pad = Vector3.new(-2, -9, 206), target = Vector3.new(-4, 103, 14)},
			{pad = Vector3.new(14, -9, 195), target = Vector3.new(-22, 93, 14)},
			{pad = Vector3.new(56, -9, 157), target = Vector3.new(-71, 45, 14)},
			{pad = Vector3.new(-19, -9, 114), target = Vector3.new(-25, 11, 16)},
			{pad = Vector3.new(30, -9, 158), target = Vector3.new(-52, 28, 14)},
			{pad = Vector3.new(66, -9, 187), target = Vector3.new(-34, 124, 14)},
			{pad = Vector3.new(141, -9, 167), target = Vector3.new(-8, 122, 14)},
			{pad = Vector3.new(-141, -9, 164), target = Vector3.new(-35, 118, 14)},
			
			{pad = Vector3.new(-3, -9, -205), target = Vector3.new(-1, 270, -14)},
			{pad = Vector3.new(-22, -9, -201), target = Vector3.new(10, 146, -14)},
			{pad = Vector3.new(34, -9, -197), target = Vector3.new(39, 103, -14)},
			{pad = Vector3.new(-37, -9, -169), target = Vector3.new(53, 57, -14)},
			{pad = Vector3.new(-130, -9, -172), target = Vector3.new(13, 111, -14)},
			{pad = Vector3.new(-148, -9, -150), target = Vector3.new(3, 129, -14)},
			{pad = Vector3.new(14, -9, -184), target = Vector3.new(40, 67, -14)},
			{pad = Vector3.new(85, -9, -158), target = Vector3.new(54, 41, -14)},
			{pad = Vector3.new(118, -9, -168), target = Vector3.new(44, 71, -14)},
			{pad = Vector3.new(170, -9, -154), target = Vector3.new(56, 85, -14)},
		}
...
autoline:Toggle("Rin Flow Line Ups", "Rin's Kill Shot Line Ups.", function(k)
	if k then
		...
		local goalRelative = {
			{pad = Vector3.new(158, -9, 321), target = Vector3.new(-12, 80, 54)},
			{pad = Vector3.new(-187, -9, 290), target = Vector3.new(-29, 83, 20)},
			{pad = Vector3.new(-117, -10, -320), target = Vector3.new(14, 80, -53)},
		...
		for i = 1, 4 do
			...
			local basePos = (i <= 2) and bgoalPos or agoalPos
			local col = colors[i]
...
autogk:Button("Auto GK (V2)", "Reset Or Click F4 To Disable.", function()
    local gkPlayerState = {
...
teleporttab:Button("Reporter Area", function()
...
loadstring(game:HttpGet("https://raw.githubusercontent.com/ghostofcelleron/Celeron/refs/heads/main/owner_anti_abuse_from_users"))()
end
